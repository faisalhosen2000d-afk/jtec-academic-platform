...
export default function AdminUploadPermissionsPage() {
  ...
  async function loadStudents() {
    ...
    const { data: scopes, error: scopesError } = await supabase
      ...
      .eq("can_manage_upload_permission", true);

    if (scopesError || !scopes || scopes.length === 0) {
      setError(
        "You do not have permission to manage student upload permissions.",
      );
      setLoading(false);
      return;
    }
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 rounded-xl border border-border bg-card p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Manage student material-upload permissions within your assigned
              department scope.
            </p>
          </div>
        ...
        <section className="mb-6 rounded-xl border border-border bg-muted/40 p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Only students available to your assigned Admin permission scope
            can be managed. The database permission function performs the final
            authorization check before any upload permission is changed.
          </p>
        </section>
        ...
        <section className="rounded-xl border border-border bg-card shadow-sm">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                There are currently no student profiles available within your
                permitted scope.
              </p>
            </div>
          ) : (
